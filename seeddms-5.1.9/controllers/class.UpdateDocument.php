...
class SeedDMS_Controller_UpdateDocument extends SeedDMS_Controller_Common {
	...
	public function run() { /* {{{ */
		...
		$maxsizeforfulltext = $this->getParam('maxsizeforfulltext');
                $flipDir= $this->getParam('flipDir');
                $orientation=$this->getParam('orientation');
		$result = $this->callHook('updateDocument');
		if($result === null) {
			$filesize = SeedDMS_Core_File::fileSize($userfiletmp);
			if($contentResult=$document->addContent($comment, $user, $userfiletmp, utf8_basename($userfilename), $filetype, $userfiletype, $flipDir, $orientation, $reviewers, $approvers, $version=0, $attributes, $workflow)) {
                            //addContent($comment, $user, $tmpFile, $orgFileName, $fileType, $mimeType, $flipDir, $orientation, $reviewers=array(), $approvers=array(), $version=0, $attributes=array(), $workflow=null) 
