...
$headerInfo = array();
$headerInfo[]=isset($_POST["page_num"]);
for($i=0; $i<$_POST["headerType"]; $i++){
...
$headerInfo[]=$headers;
$headerInfo[]=($_POST["page_num"] && $_POST["headerType"])?$_POST['headerSeq']:null;

$flipDir=$_POST["flipDir"];
...
//	}
    ...
    if (/* is_uploaded_file($_FILES["userfile"]["tmp_name"][$file_num]) && */$_FILES["userfile"]["size"][$file_num]!=0 && $_FILES['userfile']['error'][$file_num]!=0){
            UI::exitError(getMLText("folder_title", array("foldername" => $folder->getName())),getMLText("uploading_failed"));
    }
}
...
for ($file_num=0;$file_num<count($_FILES["userfile"]["tmp_name"]);$file_num++){
	...
	$fileType = ".".pathinfo($userfilename, PATHINFO_EXTENSION);
        ...
        if ($file_num==0 && $fileType!=".pdf") {
            UI::exitError(getMLText("folder_title", array("foldername" => $folder->getName())),getMLText("PDF_file_required"));
        }elseif($file_num==1 && $fileType!=".docx"){
            UI::exitError(getMLText("folder_title", array("foldername" => $folder->getName())),getMLText("Word_file_required"));
        }else if($file_num==2 && $fileType!=".pdf"){
            UI::exitError(getMLText("folder_title", array("foldername" => $folder->getName())),getMLText("PDF_file_required"));
        }
...
//	else $name = utf8_basename($userfilename);
	...
	if(!$settings->_enableDuplicateDocNames) {
        ...
        if($file_num==0 && $fileType==".pdf"){
            ...
            $controller->setParam('flipDir', $flipDir);
            $controller->setParam('orientation', $orientation);
            $controller->setParam('type', $type);
            
            $document = $controller->run();
            if(!$document) { //assign document data to $document
        ...
        }else if($file_num==1 && $fileType==".docx"){
            $res = $document->addDocumentWordFile($name, $comment, $user, $userfiletmp, utf8_basename($userfilename),$fileType, $userfiletype, 1, 1);
            if (is_bool($res) && !$res) {
		UI::exitError(getMLText("folder_title", array("foldername" => $folder->getName())),getMLText("error_occured"));
            } else {
            ...
            chdir($currentdir);
                ...
                $x = $document->getDir();
                $docdir = "../../data/1048576/$x";
                pprep($docdir, $reqversion, $headerInfo);

                //cmd to index in solr
                $cmd='../elib/solr/bin/post -c '.$settings->_solrcore.' '.$docdir.$reqversion.'/*.txt';
                
                shell_exec($cmd); 
                file_put_contents("../elib/delete.xml", $cmd);
