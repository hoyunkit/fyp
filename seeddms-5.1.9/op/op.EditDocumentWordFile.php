...
if(!checkFormKey('editdocumentwordfile')) {
...
$file = $document->getDocumentWordFile($_POST["fileid"]);

if (!is_object($file)) {
	UI::exitError(getMLText("document_title", array("documentname" => htmlspecialchars($document->getName()))),getMLText("invalid_file_id"));
}

if (($document->getAccessMode($user, 'editDocumentWordFile') < M_ALL)&&($user->getID()!=$file->getUserID())) {
