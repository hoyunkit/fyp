...
function _printMessage($heading, $message) { /* {{{ */
	...
	header("Location:../out/out.AdminLogin.php?msg=".urlencode($message));
	exit;
...
if(!$user->isAdmin() || $_SERVER['SERVER_ADDR'] != "::1" || $_SERVER['SERVER_ADDR'] != "127.0.0.1" || $_SERVER['SERVER_ADDR'] != "127.0.1.1"){
    _printMessage(getMLText("login_error_title"),	getMLText("invalid_admin"));
    exit;
}
...
if (isset($_COOKIE["mydms_session"])) {
	...
	if(!$resArr = $session->load($dms_session)) {
		...
		header("Location: " . $settings->_httpRoot . "out/out.AdminLogin.php?referuri=".$refer);
		exit;
	} else {
