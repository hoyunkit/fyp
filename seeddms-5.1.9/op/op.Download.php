...
elseif (isset($_GET["wordFile"])) { /* {{{ */
    ...
    if (!is_numeric($_GET["wordFile"]) || intval($_GET["wordFile"])<1) {
        UI::exitError(getMLText("document_title", array("documentname" => $document->getName())),getMLText("invalid_file_id"));
    }
    ...
    if(!file_exists($dms->contentDir . $wordFile->getPath())) {
        UI::exitError(getMLText("document_title", array("documentname" => $document->getName())),getMLText("missing_file"));
    }
    header("Content-Transfer-Encoding: binary");
    header("Content-Length: " . filesize($dms->contentDir . $wordFile->getPath() ));
    $efilename = rawurlencode($wordFile->getOriginalFileName());
    header("Content-Disposition: attachment; filename=\"" . $efilename . "\"; filename*=UTF-8''".$efilename);
    header("Content-Type: " . $wordFile->getMimeType());
    header("Cache-Control: must-revalidate");
    sendFile($dms->contentDir . $wordFile->getPath());
} /* }}} */
else if(isset($_GET["scannedFile"])){
    $documentid = $_GET["documentid"];
    header("Content-Transfer-Encoding: binary");
    header("Content-Length: " . filesize("../elib/data/1048576/".$documentid."/book.pdf"));
    header("Content-Disposition: attachment; filename=\"book.pdf\"; filename*=UTF-8''book.pdf");
    header("Content-Type: application/pdf");
    header("Cache-Control: must-revalidate");
    sendFile("../elib/data/1048576/".$documentid."/book.pdf");
}
