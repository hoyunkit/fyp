...
switch($command) {
	...
	case 'searchfolder': /* {{{ */
		...
		break; /* }}} */
        /*receive ajax data by data-url of div dom when folder was clicked to expand, normally show 2 layers of folder when first loading*/
//        case 'subtree':
//            if(isset($_GET['selUser']) && $_GET['selUser'] != -1){
//                $selUser=$_GET['selUser'];
//                $user=$dms->getUser($selUser);
//                $target=1;
//            }
//            else if(isset($_GET['selGroup']) && $_GET['selGroup'] != -1){
//                $selGruop=$_GET['selGroup'];
//                $user=$dms->getGroup($selGruop);
//                $target=2;
//            }
//            if($user) {
//                    if(empty($_GET['node']))
...
//                    $subfolders = SeedDMS_Core_DMS::filterAccess($subfolders, $user, M_READ, $target);
//                    $tree = array();
//                    foreach($subfolders as $subfolder) {
//                            $loadondemand = $subfolder->hasSubFolders() || ($subfolder->hasDocuments() && $showdocs); //false if no subFolder / doc
...
//		break; /* }}} */
        case 'subtree1':
//            if(isset($_GET['selUser']) && isset($_GET['selGroup']) ){
//                
//            }
            if(isset($_GET['selUser']) && $_GET['selUser'] != -1){
                $user=$dms->getUser($_GET['selUser']);
                $target=1;
            }
            else if(isset($_GET['selGroup']) && $_GET['selGroup'] != -1){
                $user=$dms->getGroup($_GET['selGroup']);
                $target=2;
            }
            if($user) {
                if(empty($_GET['node']))
                ...
                if (!is_object($folder)) return '';
                $subfolders = $folder->getSubFolders($orderby);
                $subfolders = SeedDMS_Core_DMS::filterAccessWithColor($subfolders, $user, M_READ, $target);
                $tree = array();
                foreach($subfolders as $subfolder) {
                    $loadondemand = $subfolder[0]->hasSubFolders() || ($subfolder[0]->hasDocuments() && $showdocs); //false if no subFolder / doc
                    $level = array('label'=>$subfolder[0]->getName(), 'id'=>$subfolder[0]->getID(), 'load_on_demand'=>$loadondemand, 'is_folder'=>true);
                    if(!$subfolder[0]->hasSubFolders())
                            $level['children'] = array();
                    if($subfolder[1])
                    	$level['color'] = "green";
                    else
                ...
                if($showdocs) {
                    $documents = $folder->getDocuments($orderby);
                    $documents = SeedDMS_Core_DMS::filterAccessWithColor($documents, $user, M_READ);
                    foreach($documents as $document) {
                        $level = array('label'=>$document[0]->getName(), 'id'=>$document[0]->getID(), 'load_on_demand'=>false, 'is_folder'=>false);
                        if($document[1])
                        	$level['color'] = "green";
                        else
                        	$level['color'] = "red";
					...
					if ($document->getAccessMode($user, 'removeDocument') >= M_READWRITE) {
						...
						if($controller->run()) {
							...
							add_log_line();
                                                    ...
                                                    $docid=substr($x, 0, -1);
                                                    shell_exec("rm -r ../../data/1048576/".$docid."/");
                                                    file_put_contents("../elib/delete.xml", "<delete><query>id:*/1048576/".$docid."*</query></delete>");
                                                    shell_exec("../elib/solr/bin/post -c ".$settings->_solrcore."  ../elib/delete.xml");
                                                    $res=$dms->DeleteBookmark($docid, 0, 0, 0, 1);
                                                    $res=$dms->DeleteReadHis($docid);
						} else {
							header('Content-Type: application/json');
				...
				if($document) {
						...
						if($index) {
							...
							if(!$error) {
                                ...
                                file_put_contents("../elib/delete.xml", "<delete><query>id:*/prepend.txt*</query></delete>");
                                shell_exec("../elib/solr/bin/post -c ".$settings->_solrcore."  ../elib/delete.xml");
