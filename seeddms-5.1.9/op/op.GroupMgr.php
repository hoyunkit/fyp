...
// Add user to group --------------------------------------------------------
	...
	// Glenn edited on 30 March, 2022
	$followGroupAccess = false;

	// Glenn edited on 30 March, 2022
	if (!$group->isMember($newMember)) {
		$group->addUser($newMember);
		if (isset($_POST["manager"])) {
			$group->toggleManager($newMember);
		}
		// Glenn edited on 30 March, 2022
		if (isset($_POST["followGroup"])) {
			$followGroupAccess = $group->followGroupAction($newMember);
		}
	}

	// Glenn edited on 30 March, 2022
	if ($followGroupAccess) {
		// Follow group's folder access permission operation
		$group->followGroupFolderAccessOperation($newMember);
		// Follow group's document access permission operation
		$group->followGroupDocumentAccessOperation($newMember);
	}
...
// Remove user from group --------------------------------------------------
	...
	// Edited on 31 March, 2022
	$group->removeUserFromGroupAction($oldMember);
...
} else if ($action == "editgrppermit") {
	$permit = $_POST["permit"];
	$groupid = $_POST["groupid"];
	$group = $dms->getGroup($groupid);
	$group->updateGroupPermit($permit);
	//$oldpermit=$group->getGroupPermit();
