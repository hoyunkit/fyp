...
class SeedDMS_View_ViewFolder extends SeedDMS_Bootstrap_Style {
	...
	function getAccessModeText($defMode) { /* {{{ */
        ...
        //glenn
        function getAccessModeTextByMode($defMode, $allAccesses) {
        ...
        //glenn
	function printAccessList($obj, $allAccesses) { /* {{{ */
		...
		// When the total number of users and groups assigned permissions exceeds three, it will be displayed in the form of a popup box
...
<?php
	...
	function show() { /* {{{ */
		...
		if(is_string($txt))
			...
			$this->pageNavigation($this->getFolderPathHTML($folder,false,null,true), "view_folder", $folder);
		}
...
//				$this->contentHeading(getMLText("folder_tree"));	
			...
			if ($enableClipboard) $this->printClipboard($this->params['session']->getClipboard(), $previewer);
                    ...
                    if(is_string($txt))
                            ...
                            if($user->isAdmin()) {
                                    ...
                                    // Glenn edited
                                    echo "<td>".$this->getAccessModeTextByMode($folder->getDefaultAccess(), $allAccesses)."</td>";
                                    echo "</tr>";
                                    if($folder->inheritsAccess()) {
                                            ...
                                            // Edited on 6 March, 2022
                                            $this->printAccessList($folder, $allAccesses);
                                            echo "</tr>";
                                    } else {
                                            echo "<tr>";
                                            echo "<td>".getMLText('access_mode').":</td>";
...
<?php
		...
		if(is_string($txt))
			...
			$this->contentHeading(getMLText("folder_contents"), false, true, "FolderContents");
                $this->contentContainerStart();
		$subFolders = $this->callHook('folderGetSubFolders', $folder, $orderby);
		if($subFolders === null)
			$subFolders = $folder->getSubFolders($orderby);
		...
		if ((count($subFolders) > 0)||(count($documents) > 0)){
			...
			if(is_string($txt))
				echo $txt;
			else {  print '<div id="FolderContentsRow" style="display:none;">';
	...
	//			print "<th>".getMLText("version")."</th>\n";
		...
		if(is_string($txt))
			echo $txt;
                print '<div id="FolderContentsPic">';
                print '<div style="display:inline-flex; width: 100%;">';
                $i=1;
                foreach($subFolders as $subFolder) {
                    echo $this->folderListPic($subFolder);
                    if($i%5==0){
                        print'</div>';
                        print '<div style="display:inline-flex; width: 100%;">';
                    }
