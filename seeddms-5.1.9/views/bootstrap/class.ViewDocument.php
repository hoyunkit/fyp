...
class SeedDMS_View_ViewDocument extends SeedDMS_Bootstrap_Style {
	...
	protected function getAccessModeTextByMode($defMode, $allAccesses)
	{
            foreach ($allAccesses as $accessItem) {
                    if ($defMode === $accessItem['mode']) {
                            return $accessItem['name'];
                    }
            }
	}
        
	protected function printAccessList($obj, $allAccesses)
	{ /* {{{ */
		...
		for ($i = 0; $i < count($accessList["groups"]); $i++) {
			...
			// $accesstext is the permission name
			// Glenn edited on 12 May, 2022
			$accesstext = $this->getAccessModeTextByMode($accessList["groups"][$i]->getMode(), $allAccesses);
		...
		for ($i = 0; $i < count($accessList["users"]); $i++) {
			...
			// $accesstext is the permission name
			// Glenn edited on 12 May, 2022
			$accesstext = $this->getAccessModeTextByMode($accessList["users"][$i]->getMode(), $allAccesses);
...
<?php
	...
	function documentInfos() { /* {{{ */
		...
		$user = $this->params['user'];
		$document = $this->params['document'];
                // Get all accesses from database
		// Glenn edit on 10 May, 2022
...
<?php
		...
		if($user->isAdmin()) {
			...
			// Glenn edited on 10 May, 2022
                        echo "<td>" . $this->getAccessModeTextByMode($document->getDefaultAccess(), $allAccesses) . "</td>";
			echo "</tr>";
			if($document->inheritsAccess()) {
				...
				$this->printAccessList($document, $allAccesses);
				echo "</tr>";
			} else {
				echo "<tr>";
				echo "<td>".getMLText('access_mode').":</td>";
...
<?php
        ...
        function actionIcon($action){
            switch($action){
                case 'edit_document_access':
                    $icon='icon-edit';
                    break;
                case 'edit_document_props':
                    $icon='icon-edit-sign';
                    break;
                case "edit_existing_notify":
                    $icon='icon-bell';
                    break;
                case 'expires':
        ...
        function getVersionInfo($docContent, $documentid, $version){
            $info='<li>'.getMLText('flip_dir').': ';
            if($docContent->getFlipDir()=='rtl'){
                $info.=getMLText('flip_right').'</li>';
            }else{
                $info.=getMLText('flip_left').'</li>';
            }
            $info.='<li>'.getMLText('orientation').': ';
            if($docContent->getOrientation()==1){
                $info.=getMLText('portrait').'</li>';
            }else if($docContent->getOrientation()==2){
                $info.=getMLText('landscape').'</li>';
            }
            $info.='<li>'.getMLText('total_page').': ';
            $dir = '../elib/data/1048576/'.$documentid.'/'.$version.'/';
            $info.=count(glob($dir. "*.jpg")).'</li>';
            return $info;
        }
	function preview() { /* {{{ */
		$dms = $this->params['dms'];
...
<?php
	...
	function show() { /* {{{ */
		...
		$showLinkedFiles = 0; // Attachment
		$showLinkedWordFiles = 0; // Word Files
		$showLinkedDocument = 0; // Related Documents
		...
		// Glenn edited on 3 May, 2022
		// Determine if 'change_status', 'edit_comment', 'current_version', 'linked_files', 'linked_wordFiles', 'linked_documents' are allowed for this user
		foreach ($privilegesObj as $priKey => $priVal) {
			// If 'change_status' privilege exists
			if ($priKey === 'change_status') {
				$showChangeStatus = 1;
			}
			// If 'edit_comment' privilege exists
			if ($priKey === 'edit_comment') {
				$showEditComment = 1;
			}
			// If 'current_version' privilege exists
			if ($priKey === 'current_version') {
				$showCurrentVersion = 1;
			}
			// If 'linked_files' privilege exists
			if ($priKey === 'linked_files') {
				$showLinkedFiles = 1;
			}
			// If 'linked_wordFiles' privilege exists
			if ($priKey === 'linked_wordFiles') {
				$showLinkedWordFiles = 1;
			}
			// If 'linked_documents' privilege exists
			if ($priKey === 'linked_documents') {
				$showLinkedDocument = 1;
			}
		}
...
<?php
		...
		$files = SeedDMS_Core_DMS::filterDocumentFiles($user, $files);
                ...
                foreach($contents as $content) {
                    $wordFiles = array_merge($wordFiles, $document->getDocumentWordFiles($content->getVersion()));
                }
//                $wordFiles = $document->getDocumentWordFiles($latestContent->getVersion());
		$wordFiles = SeedDMS_Core_DMS::filterDocumentWordFiles($user, $wordFiles);
...
<div class="span8"  id="span8mainContent">
    <ul class="nav nav-tabs" id="docinfotab">
		  <li class="<?php if(!$currenttab || $currenttab == 'docinfo') echo 'active'; ?>">
                      <?php // Glenn Edited on 3 May, 2022 
                        ?>
                        <?php
                        if ($showCurrentVersion === 1) {
                                echo '<a data-target="#docinfo" data-toggle="tab">';
                                printMLText("current_version");
                                echo '</a>';
                        }
                        ?>
                  </li>
...
?>
		  <li class="<?php if($currenttab == 'attachments') echo 'active'; ?>">
                      <?php // Glenn Edited on 3 May, 2022 
                            ?>
                        <?php
                        if ($showLinkedFiles === 1) {
                                echo '<a data-target="#attachments" data-toggle="tab">';
                                printMLText('linked_files');
                                echo (count($files)) ? " (" . count($files) . ")" : "";
                                echo '</a>';
                        }
                        ?>
                  </li>
                  <li class="<?php if($currenttab == 'wordFiles') echo 'active'; ?>">
                      <?php // Glenn Edited on 3 May, 2022 
                        ?>
                        <?php
                        if ($showLinkedWordFiles === 1) {
                                echo '<a data-target="#wordFiles" data-toggle="tab">';
                                printMLText('linked_wordFiles');
                                echo (count($wordFiles)) ? " (" . count($wordFiles) . ")" : "";
                                echo '</a>';
                        }
                        ?>
                  </li>
                  <li class="<?php if($currenttab == 'scannedFiles') echo 'active'; ?>"><a data-target="#scannedFiles" data-toggle="tab"><?php printMLText('linked_scannedFiles');?></a></li>
			<li class="<?php if($currenttab == 'links') echo 'active'; ?>">
                            <?php // Glenn Edited on 3 May, 2022 
                                ?>
                                <?php
                                if ($showLinkedDocument === 1) {
                                        echo '<a data-target="#links" data-toggle="tab">';
                                        printMLText('linked_documents');
                                        echo (count($links) || count($reverselinks)) ? " (" . count($links) . "/" . count($reverselinks) . ")" : "";
                                        echo '</a>';
                                }
                                ?>
                        </li>
...
<?php
		...
		print "<thead>\n<tr>\n";
		print "<th width='20%'>".getMLText("preview")."</th>\n";
		print "<th width='*'>".getMLText("file_info")."</th>\n";
		print "<th width='20%'>".getMLText("comment")."</th>\n";
		print "<th width='15%'>".getMLText("status")."</th>\n";
		print "<th width='20%'>".getMLText("action")."</th>\n";
...
//		}else 
//                    print "<li><img class=\"mimeicon\" src=\"".$this->getMimeIcon($latestContent->getFileType())."\" title=\"".htmlspecialchars($latestContent->getMimeType())."\"></li>";
...
//                $page=1;
                ...
                if ($file_exists && $document->getType()==1) { 
                    print "<a draggable=\"false\" href=\"../out/out.PreviewDocument.php?documentid=".$documentid."&version=". $version."\">"; //"&page=".$page.
                }else if($document->getType()==0){
                    print "<a draggable=\"false\" href=\"../op/op.ViewOnline.php?documentid=".$documentid."&version=". $latestContent->getVersion()."\" target=\"_blank\">";
                }
                
                print("<img draggable='false' title='Preview Document' src='../elib/data/1048576/$documentid/$version/1.jpg' style='width:70%;height:auto;'>");
		...
		print "<li>".getLongReadableDate($latestContent->getDate())."</li>";
                print $this->getVersionInfo($latestContent, $documentid, $version);
		print "</ul>\n";
		print "<ul class=\"actions unstyled\">\n";
...
//                $page=($readingDetail)?$readingDetail["page"]:1;
                $page=1;
//		if ($file_exists){
...
//                                print "<li><a href=\"../out/out.PreviewDocument.php?documentid=".$documentid."&version=". $latestContent->getVersion()."&page= ".$page."\"><i class=\"icon-star\"></i>" . "Preview Document" . "</a></li>";
//                        
//		}
//		print "</ul>";
		print "<ul class=\"unstyled actions\" style=\"font-size: 16px;\">";
                $menuitems=$this->docActions($document);
                foreach($menuitems as $menuitem) {
                    $icon=$this->actionIcon($menuitem['label']);
                    echo "<li><a href=\"".$menuitem['link']."\"><i class=\"".$icon."\"></i>".getMLText($menuitem['label'])."</a></li>";
		}
		if ($file_exists){
		...
		if($accessop->mayRemoveVersion()) {
			print "<li><a href=\"../out/out.RemoveVersion.php?documentid=".$documentid."&version=".$latestContent->getVersion()."\"><i class=\"icon-remove\"></i>".getMLText("rm_version")."</a></li>";
                ...
                // Glenn edited on 3 May, 2022
                // If 'change_status' is allowed, show it
		...
		*/
                // Glenn edited on 3 May, 2022
                // If 'edit_comment' is allowed, show it
                if ($showEditComment === 1) {
...
?>
                  ...
                  <!--chenrui wordFile-->
                  <div class="tab-pane <?php if($currenttab == 'wordFiles') echo 'active'; ?>" id="wordFiles">
<?php
    $this->contentContainerStart();
    if (count($wordFiles) > 0) {
        print "<table class=\"table\">";
        print "<thead>\n<tr>\n";
        print "<th width='20%'></th>\n";
        print "<th width='20%'>".getMLText("wordFile")."</th>\n";
        print "<th width='40%'>".getMLText("comment")."</th>\n";
        print "<th width='20%'></th>\n";
        print "</tr>\n</thead>\n<tbody>\n";

        foreach($wordFiles as $wordFile) {
            $version=$wordFile->getVersion();
            $file_exists=file_exists($dms->contentDir . $wordFile->getPath());
            $responsibleUser = $wordFile->getUser();
            print "<tr>";
            print "<td>";
            $previewer->createPreview($file, $previewwidthdetail); //generate image
            if($file_exists) {
                if ($viewonlinefiletypes && in_array(strtolower($wordFile->getFileType()), $viewonlinefiletypes))
                    print "<a target=\"_blank\" href=\"../op/op.ViewOnline.php?documentid=".$documentid."&wordFile=". $wordFile->getID()."\">";
                else if(($document->getAccessMode($user) == M_ALL)||($wordFile->getUserID()==$user->getID()))
                    print "<a href=\"../op/op.Download.php?documentid=".$documentid."&wordFile=".$wordFile->getID()."\">";
            }
            if($previewer->hasPreview($file)) {
                    print("<img class=\"mimeicon\" width=\"".$previewwidthdetail."\" src=\"../op/op.Preview.php?documentid=".$document->getID()."&wordFile=".$wordFile->getID()."&width=".$previewwidthdetail."\" title=\"".htmlspecialchars($wordFile->getMimeType())."\">");
            } else {
                    print "<img class=\"mimeicon\" width=\"".$previewwidthdetail."\" src=\"".$this->getMimeIcon($wordFile->getFileType())."\" title=\"".htmlspecialchars($wordFile->getMimeType())."\">";
            }
            if($file_exists) {
                print "</a>";
            }
            ...
            print "<td><ul class=\"unstyled\">\n";
            print "<li>".htmlspecialchars($wordFile->getName())."</li>\n";
            if($wordFile->getName() != $wordFile->getOriginalFileName())
                print "<li>".htmlspecialchars($wordFile->getOriginalFileName())."</li>\n";
            if ($file_exists)
                print "<li>".SeedDMS_Core_File::format_filesize(filesize($dms->contentDir . $wordFile->getPath($version))) ." bytes, ".htmlspecialchars($wordFile->getMimeType())."</li>";
            else print "<li>".htmlspecialchars($wordFile->getMimeType())." - <span class=\"warning\">".getMLText("document_deleted")."</span></li>";

            print "<li>".getMLText("uploaded_by")." <a href=\"mailto:".$responsibleUser->getEmail()."\">".htmlspecialchars($responsibleUser->getFullName())."</a></li>";
            print "<li>".getLongReadableDate($wordFile->getDate())."</li>";
            if($version)
                    print "<li>".getMLText('link_to_version')." ".$version."</li>";
            else
                    print "<li>".getMLText('linked_to_document')."</li>";
            print "</ul></td>";
            print "<td>".htmlspecialchars($wordFile->getComment())."</td>";

            print "<td><ul class=\"unstyled actions\">";
            if (($document->getAccessMode($user) == M_ALL)||($wordFile->getUserID()==$user->getID())) { //$file_exists
                    print "<li><a href=\"../op/op.Download.php?documentid=".$documentid."&wordFile=".$wordFile->getID()."\"><i class=\"icon-download\"></i>".getMLText('download')."</a>";
                    if ($viewonlinefiletypes && in_array(strtolower($wordFile->getFileType()), $viewonlinefiletypes))
                            print "<li><a target=\"_blank\" href=\"../op/op.ViewOnline.php?documentid=".$documentid."&wordFile=". $wordFile->getID()."\"><i class=\"icon-star\"></i>" . getMLText("view_online") . "</a></li>";
            }
            echo "</ul><ul class=\"unstyled actions\">";
            if (($document->getAccessMode($user) == M_ALL)||($wordFile->getUserID()==$user->getID())) {
                    print "<li><a href=\"out.RemoveDocumentWordFile.php?documentid=".$documentid."&fileid=".$wordFile->getID()."\"><i class=\"icon-remove\"></i>".getMLText("delete")."</a></li>";
                    print "<li><a href=\"out.EditDocumentWordFile.php?documentid=".$documentid."&fileid=".$wordFile->getID()."\"><i class=\"icon-edit\"></i>".getMLText("edit")."</a></li>";
            }
            print "</ul></td>";		
...
else printMLText("no_word_files");

if ($document->getAccessMode($user) >= M_READWRITE){
        print "<ul class=\"unstyled\"><li><a href=\"../out/out.AddWordFile.php?documentid=".$documentid."\" class=\"btn\">".getMLText("add")."</a></ul>\n";
}
$this->contentContainerEnd();
?>
  </div>
     <div class="tab-pane <?php if($currenttab == 'scannedFiles') echo 'active'; ?>" id="scannedFiles">
<?php
    $this->contentContainerStart();
    $file_exists=file_exists("../elib/data/1048576/".$documentid."/book.pdf");
    if ($file_exists) {
        print "<table class=\"table\">";
        print "<thead>\n<tr>\n";
        print "<th width='20%'></th>\n";
...
//        print "<th width='40%'>".getMLText("comment")."</th>\n";
        ...
        print "</tr>\n</thead>\n<tbody>\n";
            print "<tr>";
            print "<td>";
            print "<a target=\"_blank\" href=\"../op/op.ViewOnline.php?documentid=".$documentid."&scannedFile=book\">";
            print "<img class=\"mimeicon\" width=\"".$previewwidthdetail."\" src=\"".$this->getMimeIcon(".pdf")."\" title=\"".htmlspecialchars("application/pdf")."\">";
            print "</a>";
            print "</td>";

            print "<td><ul class=\"unstyled actions\">";
            if (($user->isAdmin())) { //$file_exists
                    print "<li><a href=\"../op/op.Download.php?documentid=".$documentid."&scannedFile=book\"><i class=\"icon-download\"></i>".getMLText('download')."</a>";
            }
            echo "</ul><ul class=\"unstyled actions\">";
            if (($user->isAdmin())) {
                    print "<li><a href=\"out.RemoveDocumentScannedFile.php?documentid=".$documentid."&fileid=book\"><i class=\"icon-remove\"></i>".getMLText("delete")."</a></li>";
            }
            print "</ul></td>";		
...
else printMLText("no_scanned_files");

if ($user->isAdmin()){
        print "<ul class=\"unstyled\"><li><a href=\"../out/out.AddScannedFile.php?documentid=".$documentid."\" class=\"btn\">".getMLText("add")."</a></ul>\n";
}
$this->contentContainerEnd();
?>
  </div>               
