...
?>
	...
	$('#presetexpdate').on('change', function(ev){
        ...
        $("#headerType").change(function(e){
            var i, type, header, parent; 
            type = $("#headerType").val();
            $('.headerContainer').hide();
            $('.seqContainer').hide();
            for(i=1; i<=type; i++){
                $('#headerContainer'+i).show();
                if($('[name="page_num"]').is(":checked")){
                    $('.seqContainer').show();
                }
            }
        });
        
        $('[name="page_num"]').change(function(e){
            if(e.target.checked && $("#headerType").val()>0){
                $('.seqContainer').show();
            }else{
                $('.seqContainer').hide();
            }
...
<?php
		...
		$this->formField(
			...
			array(
                ...
                $options = array([1, getMLText("document")], [0, getMLText("user_guide")]);
		$this->formField(
			getMLText("type"),
			array(
				'element'=>'select',
		...
		$this->formField(
			getMLText("pdf_file"),
			$enablelargefileupload ? $this->getFineUploaderHtml() : $this->getFileChooserHtml('userfile[]', false, '.pdf').($enablemultiupload ? '<a class="" id="new-file"><?php printMLtext("add_multiple_files") ?></a>' : '')
		);
                $this->formField(
			getMLText("wordFile")." ".getMLText("optional"),
			$enablelargefileupload ? $this->getFineUploaderHtml() : $this->getFileChooserHtml('userfile[]', false, '.docx,.doc').($enablemultiupload ? '<a class="" id="new-file"><?php printMLtext("add_multiple_files") ?></a>' : '')
		);
                $this->formField(
			getMLText("ScannedFile")." ".getMLText("optional"),
			$enablelargefileupload ? $this->getFineUploaderHtml() : $this->getFileChooserHtml('userfile[]', false, '.pdf').($enablemultiupload ? '<a class="" id="new-file"><?php printMLtext("add_multiple_files") ?></a>' : '')
		);
                $options = array(['ltr', getMLText("flip_left")], ['rtl', getMLText("flip_right")]);
		$this->formField(
			getMLText("flip_dir"),
			array(
				'element'=>'select',
                ...
                $options = array([1, getMLText("portrait")], [2, getMLText("landscape")]);
		$this->formField(
			getMLText("orientation"),
			array(
				'element'=>'select',
				'id'=>'orientation',
                                'class'=>'chzn-select',
		...
		$this->formField(
			...
			array(
                ...
                $this->contentSubHeading(getMLText("header_info"));
                $this->formField(
			getMLText("has_page_num"),
			array(
				'element'=>'input',
				'type'=>'checkbox',
				'name'=>'page_num',
		...
		$this->formField(
			...
			array(
                ...
                for ($index = 1; $index <= 5; $index++) {
                    print "<div class=\"headerContainer\" id=\"headerContainer".$index."\" hidden>";
                    $this->formField(
                            getMLText("header")." (".$index.")",
                            array(
                                    'element'=>'input',
                                    'type'=>'text',
                                    'class'=>'header',
                ...
                print "<div class=\"seqContainer\" hidden>";
                $options = array([1, getMLText("header_before")], [2, getMLText("header_after")]);
		$this->formField(
			getMLText("header_seq"),
			array(
				'element'=>'select',
				'id'=>'headerSeq',
                                'class'=>'chzn-select',
