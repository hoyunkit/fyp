                ...
                print"$(document).ready( function() {".
                        "$( window ).resize(function() {
                            ...
                            if(window.innerWidth<1347){
                                ...
                                $('.row-fluid .span8').removeAttr('style');
                                $('.row-fluid .span4').removeAttr('style');
                            }
                        });
                        
                        $('#resulttbl > tbody > tr').live('click',function(){
                            var query=$('input[name=query]').val();
                            var docid=$(this).attr('rel').replace('document_','');
                            var id=$(this).attr('id');
                            var ver=$(this).attr('ver');
                            
                            if($('#'.concat(id)).hasClass('expand')){
                                 $('#'.concat(id)).removeClass('expand');
                                 $('#'.concat(id)).addClass('fold');
                                 $('#'.concat('docresponse').concat(docid)).hide();
                                 return;
                            }
                            if($('#'.concat(id)).hasClass('fold')){
                                 $('#'.concat(id)).removeClass('fold');
                                 $('#'.concat(id)).addClass('expand');
                                 $('#'.concat('docresponse').concat(docid)).show();
                                 return;
                            }
                            ...
                            $.ajax({
                                ...
                                data: {'action': 'search', 'query': query, 'docid': docid,'mode':1,'version':ver},
                                success:function(response){
                                    //$('.table').append(response);
                                   // alert(tmp.children('td:eq(1)').text());
                                    $('#'.concat(id)).addClass('expand');
                                    var tmp=$('#'.concat(id)).children('td:eq(1)');
                                    //alert(response);
                                    tmp.html(tmp.html().concat(response));
                                }
                            });

                        });
                        $('#resultForm').live('click', function(){
                            $(this).submit();
                        });
                        $('#topnavbar').css('display', 'block');
                        $(window).resize();
//                        $('.span4 tr td:nth-child(2)').css('width','80%');
                        $('.span4 tr td:nth-child(2)').css('display','block');
                        $('#choosefoldersearchform1').css('width','60%');".
                "});";
	...
	function show() { /* {{{ */
		...
		$this->contentStart();
                print '<div class="span12" id="maincontent-container" style="padding:50px 50px 20px 50px;">';
...
<?php
	...
	$this->contentContainerStart();
?>
<form action="../out/out.Search.php" name="form2" >
...
//			$this->contentContainerStart();
			...
			if(is_string($txt))
				...
				print "<table id='resulttbl' class=\"table table-hover\">";
				print "<thead>\n<tr>\n";
				print "<th></th>\n";
			...
			foreach ($entries as $entry) {
				if(get_class($entry) == $dms->getClassname('document')) {
					...
					if(is_string($txt))
						...
						print "<tr id=\"table-row-document-".$document->getID()."\" class=\"table-row-document\" rel=\"document_".$document->getID()."\" formtoken=\"".createFormKey('movedocument')."\" ver='".$version."' style='cursor:pointer;'>";
						...
						print "</a></td>"; */
                                                print"<td></td>";
						//print "<td><a class=\"standardText\" href=\"../out/out.ViewDocument.php?documentid=".$document->getID()."\">";
						//print "<td><a class=\"standardText\" href=\"../out/out.ViewDocumentfromSearch.php?documentid=".$document->getID()."&position=".$document->position ."\">"; 
						//print $docName."</a>";
                                                print "<td>".$docName;
						...
						print "</span>";
                                                //print "<br />  <a href=\"../out/out.PreviewDocument.php?documentid=".$document->getID()."&version=".$document->ver."&page=".$document->page.
                                                        //"&query=".$this->query."\"><span style=\"font-size: 85%; font-style: italic; color: #666; \">"."page:".$document->page."<br>".$document->hl." </span></a>";
