...
?>
		...
		$(document).ready( function() {
			...
			$('#addmember-btn').live('click', function(e) {
				e.preventDefault ? e.preventDefault() : (e.returnValue = false);
				const userid = $('#addmember-userid').val();
				if (userid == -1) {
					noty({
						text: 'Member field cannot be empty',
						type: 'error',
						dismissQueue: true,
						layout: 'topRight',
						theme: 'defaultTheme',
	...
	{ /* {{{ */
		...
		?>
			...
			<form class="form-inline" action="../op/op.GroupMgr.php" method="POST" name="form_2" id="form_2" style="display: flex; align-items: center; justify-content: flex-start;">
				<?php echo createHiddenFieldWithKey('addmember'); ?>
				<input type="Hidden" name="action" value="addmember">
				<input type="Hidden" name="groupid" id="addmember-groupid" value="<?php print $group->getID(); ?>">
				<div style="display: flex; flex-direction: column;">
					<select name="userid" id="addmember-userid">
					...
					<label class="checkbox" style="margin: 12px 0 0; user-select: none;"><input type="checkbox" name="manager" id="addmember-manager" value="1"><?php printMLText("manager"); ?></label>
					<label class="checkbox" style="margin: 6px 0 12px; user-select: none;"><input type="checkbox" name="followGroup" id="followGroupAction" value="1"><?php printMLText("follow_group"); ?></label>
					<input type="button" class="btn" id="addmember-btn" value="<?php printMLText("add"); ?>" style="width: 73px;" />
				</div>
			</form>
		<?php
		}
