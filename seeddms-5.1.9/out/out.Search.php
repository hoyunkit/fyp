...
// Search in Fulltext {{{
	...
	if(isset($_GET['categoryids']) && $_GET['categoryids']) {
		foreach($_GET['categoryids'] as $catid) {
			if($catid > 0) {
                ...
                $cat_docid=$dms->getDocumentIDbyCategoriesID($_GET['categoryids']);
	}
	...
	if (isset($_GET["folderid"]) && $_GET["folderid"]) {
		$folderid = $_GET["folderid"];
		$fileinfol=$dms->getfileinfolder($folderid);
	}
	//var_dump($fileinfol);
	...
	if(strlen($query) < 4 && strpos($query, '*')) {
		...
		$lucenesearch = new $indexconf['Search']($index);
                ...
                for($i=0; $i<count($tmpid); $i++){
                    if(!isset($cat_docid) || in_array($tmpid[$i]['id'], $cat_docid)){
                        if (!isset($owner) ||$_GET["ownerid"]==$dms->getOwnerIDbyDocumentID($tmpid[$i]['id'])){
                        	if(!isset($_GET["folderid"])||in_array($tmpid[$i]['id'], $fileinfol)){
	                           $solrre=callSolr($query,$tmpid[$i]['id'],0, $settings->_solrcore,'0', $settings->_contentDir);
	                            if($solrre!=NULL){
	                                $hits = array_merge($hits, $solrre);
	                          } 
                        	}
