...
$flipDir=$latestContent->getFlipDir();
$orientation=$latestContent->getOrientation();
...
if ($document->verifyLastestContentExpriry()){
	header("Location:../out/out.PreviewDocument.php?documentid=".$document->getID());
	exit;
}
//file_put_contents("../elib/data/log.txt", $user->getID()."|".$document->getID()."|".date("Y-m-d")."|".date("H:i:s")."\n", FILE_APPEND);
...
if($view) {
	...
	$view->setParam('page', isset( $_GET['page'])? $_GET['page']:$_POST['page']);
        $view->setParam('version', $_GET['version']);
        $view->setParam('query',isset( $_GET['query'])? $_GET['query']:$_POST['query']);
        $view->setParam('flipDir', $flipDir);
        $view->setParam('orientation', $orientation);
        $view->setParam('tabStatus', isset($_GET['tabStatus'])?$_GET['tabStatus']:$_POST['tabStatus']);
	$view($_GET);
	exit;
}
