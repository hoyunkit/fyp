<?php
    ...
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] == 'addbookmarknew') {
            // 处理添加书签请求
            $documentid = $_POST['documentid'];
            $userid = $user->getID();
            $page = $_POST['page'];
            ...
            $add = $dms->AddBookmarkNew($documentid, $userid, $page, 1, $note);
...
//            echo $str;
            ...
            $documentid=$_POST['documentid'];            
            $version = 1;
            $showbookmark = $dms->GetBookmarkNew($documentid, $userid, $version);

            echo json_encode($showbookmark);
        ...
        //file_put_contents("/home/www-data/seeddms51x/debug.txt", "Click".$_POST['comment'], FILE_APPEND);
        ...
        }else if ($_POST['action'] == 'showhighlightinfo') {
			$userid = $user->getID();
			$documentid = $_POST['documentid'];
			$pageno = $_POST['page'];
			$showhighlightinfo = $dms->GetHighlightInfo($documentid, $userid, $pageno);
		    //$showhighlightinfo = $dms->GetHighlightInfo(310,1,4);
			echo json_encode($showhighlightinfo);
		  }else if($_POST['action']=='delbookmark'){
            ...
            $version=$_POST['version'];*/
            $page=$_POST['page'];
            $del=$dms->DeleteBookmarkNew($documentid, $page);
        ...
        else if($_POST['action'] == 'deleteallbookmarks') {
            $documentId = $_POST['documentid'];
            $userid = $user->getID();
            $deleteAll = $dms->DeleteAllBookmarks($documentId,$userid);
          ...
          else if ($_POST['action'] == 'selectdeletebookmarks') {
            $documentId = $_POST['documentid'];
            $selectedBookmarks = $_POST['selectedbookmarks'];
            $userid = $user->getID();
            $deleteSelected = $dms->DeleteSelectedBookmarks($documentId, $selectedBookmarks,$userid);
        ...
        else if($_POST['action']=='renderpdf'){
            $docid=$_POST['documentid'];
            if (file_exists('../elib/data/1048576/'.$docid.'/1.pdf')) {
                $pdfPath = 'http://localhost/nbp/e-library/data/1048576/' . $docid . '/1.pdf';
                echo $pdfPath;
            }else{
                var_dump(file_exists($pdfPath));
                echo '';
            }
        }else if($_POST['action']=='renderpdf_double'){
            $docid=$_POST['documentid'];
            if (file_exists('../elib/data/1048576/'.$docid.'/2.pdf')) {
                $pdfPath = 'http://localhost/nbp/e-library/data/1048576/' . $docid . '/2.pdf';
                echo $pdfPath;
            }else{
                var_dump(file_exists($pdfPath));
                echo '';
        ...
        else if ($_POST['action'] == 'rendernextpdf') {
            $docid = $_POST['documentid'];
            $page = $_POST['page'];
            $encryption_key = bin2hex(random_bytes(32));
          
            if (file_exists('../elib/data/1048576/' . $docid . '/1/p/' . $page . '.pdf')) {
              $pdfPath = 'http://localhost/nbp/e-library/data/1048576/' . $docid . '/1/p/' . $page . '.pdf';
...
//            echo $str;
            $userid=$user->getID();
            $documentid=$_POST['documentid'];         
            $input = array('dms' => $dms, 'userid' => $userid, 'documentid' => $documentid, 'user' => $user);
            
            $noteController = Controller('note', $input);
            $noteController->show();
        }
...
//        }
            ...
            //[docId, version, pageno, pos_left, pos_top, pos_width, pos_height, text]
            $data=$_POST['data']; 
            $res=$dms->AddHighlights($data, $data[3], $data[4], $data[5], $data[6], $data[7], $user->getID()); 
            echo 1;
        }
        else if($_POST['action']=='gethighlights'){
            //[docId, version, pageno]
            $data=$_POST['data'];
            $res=$dms->GetHighlights($data[0], $data[1], $data[2], $user->getID());
            echo json_encode($res);
        }
        else if($_POST['action']=='removehighlights'){
            $data=$_POST['data'];
            //[docId, version, pos_left, pos_top, pos_width, pos_height, pageno]
            $res=$dms->RemoveHighlights($data, $data[2], $data[3], $data[4], $data[5] ,$user->getID());
            echo 1;
        }
        else if($_POST['action']=='callgemini'){
            $data=$_POST['data'];
            //[docId, version, pos_left, pos_top, pos_width, pos_height, pageno]
            $res=$dms->RemoveHighlights($data, $data[2], $data[3], $data[4], $data[5] ,$user->getID());
            echo 1;
        ...
        //chenrui
            ...
            $page=$_POST['page'];
            $mode= $_POST['mode'];
            $user->updateReadingHistory(0, $docid, $page,$mode);
            
        }else if($_POST['action']=='recordTab'){
            $docid=$_POST['documentid'];
            $tabStatus=$_POST['tabStatus'];
            $mode= $_POST['mode'];
            $user->updateReadingHistory(1, $docid, $page,$mode);
        }else if($_POST['action']=='recordDisPage'){
            $docid=$_POST['documentid'];
            $disPage=$_POST['disPage'];
            $mode= $_POST['mode'];
            $user->updateReadingHistory(2, $docid, $page,$mode);
        }else if($_POST['action']=='agree'){
            $user->setFirstLogin();
        }else if($_POST['action']=='getRecordPage'){
            $docid = $_POST['documentid'];
            $record=$user->getReadingDetail($docid);
            echo json_encode($record);
        }else if($_POST['action']=='createRecordPage'){
            $docid = $_POST['documentid'];
            $user->addReadingHistory($docid);
        }
    }
