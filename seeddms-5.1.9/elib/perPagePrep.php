...
function findLines($lines, $paras){
    ...
    while($i<count($paras) && $j<count($lines)){
        if($i===2 && $j===18)
            $a=1;
        $hitLine=seperateStr2($line, $para, $text);
        if(!$hitLine || $line===''){
            $line=$lines[++$j];
        }
        if($para===''){
...
function seperateStr(&$para, &$index, &$text, $nextIndex){
    if($index!==''){
        ...
        if($head && preg_match('/^ *'.$head.'/u', $index) && $tail){ //find match
            $tmp=preg_split('/'.$tail.'/u', $index, 2);
            $text.=$para."\n";
            $index=$tmp[1];
            $para='';
            return true;
        }else if($head && preg_match('/^ *'.$head.'/u', $index) && !$tail){ // para seperated
            $indexTail=delimiter($index, $para, -1);
            if($indexTail){
                $tmp=preg_split('/'.$indexTail.'/u', $para, 2);
                $text.=$tmp[0].stripslashes($indexTail);
                $para=$tmp[1];
                $index='';
                return true;
                ...
                if($nextIndexHead){
                    $tmp=preg_split('/'.$nextIndexHead.'/u', $para, 2);
                    $text.=$tmp[0]."\n";
                    $para=stripslashes($nextIndexHead).$tmp[1];
                    $breakPoint=delimiter($tmp[0], $index, -1);
                    $Tmp=preg_split('/'.$breakPoint.'/u', $index, 2);
                    $index=$Tmp[1];
                    return true;
                }
            }
...
//    $border=($flag===0)?mb_substr($para, 0, 1):mb_substr($para, $flag, 1);
    while($borderLen<=5){ //傳經史略 p142 "。乃回示曰。"
