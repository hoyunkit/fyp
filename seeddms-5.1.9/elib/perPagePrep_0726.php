...
function findLines($lines, $paras, $key){ //$lines means the line array in a page
    $i=$j=0;
    $text='';
    $para=$paras[0];
    $line=$lines[0];
    while($i<count($paras) && $j<count($lines)){
        if($line!='' && $para!=''){
            if(($hit=seperateStr2($line, $para, $text)) !== true){ //image txt & extra para words
                if(!imageTxt($paras, $i, $para, $line)){ //check is the line is image txt
                    extraWord($para, $text);
                }
            }
        }
        $prePara=$para; //previous para
        $preLine=$line; //previous line
        if($line==''){
            $j++;
            $line=($j<count($lines))?$lines[$j]:'';
        }
        if($para==''){
           $i++;
           $para=($i<count($paras))?$paras[$i]:''; 
           $text.="\n";
        }
//        if($prePara==$para && $preLine==$line){
//            exit("Error occurs in page". $key+1 .'\n line: ' . $line . '\n para:' . $para);
//        }
    }
    return $text;
}
...
function printLines($line2DArr, $para2DArr, $verID){
    foreach($line2DArr as $key => $lines ) { //the keys for $line2DArr and $para2DArr are the same, which means the page num
        $paras=$para2DArr[$key];
        $text=findLines($lines, $paras, $key);
    ...
    while($i<count($paraArr) && $j<count($indexArr)){
        ...
        if($para!='' && $index!=''){
            if(($hitbuf=checkBuf($bufArr, $index, $text, $nextIndex))===false){ //check para buffer
                if(($hitPara=seperateStr($para, $index, $text, $nextIndex))===false){ // use current para to match index page
                    if(!imageTxt($paraArr, $i, $para, $index)){
                        outOrder($bufArr, $paraArr, $i, $para, $text, $index, $nextIndex);
        ...
        $prePara=$para; // record previous para
        $preIndex=$index; //previous index
...
//        if($para!="\n" && $prePara==$para && $preIndex==$index){ //both previous para and index are same as the current ones, exit the program
//            exit("Error occurs in page". $j+1 .'\n index: ' . $index . '\n para:' . $para);
//        }
    }
    return $paraRes;
}
...
function checkBuf(&$bufArr, &$index, &$text, $nextIndex){
    if(is_array($bufArr)){
        foreach ($bufArr as $k => $buf) {
            if((seperateStr($buf, $index, $text, $nextIndex))===1){
                if($buf!=''){
                   $bufArr[$k]=$buf;
                }else{
...
function outOrder($bufArr, $paraArr, &$i, &$para, &$text, &$index, $nextIndex){
    ...
    $newBufArr=$bufArr;
    $newBufArr[]=$para;
    while($count<7){ //jumping 5 para
        $paraTmp=$paraArr[$count+$i];
        if(seperateStr($paraTmp, $index, $text, $nextIndex)){
            $para=$paraTmp;
            $i+=$count;
            return $newBufArr; //find matched para, update buf
        }else{
            $newBufArr[]=$paraTmp;
        }
        $count++;
    }
    //The current para is not out of order
    //the para is extra para
    extraWord($para, $text);
    return $bufArr; 
}

function extraWord(&$para, &$text){
    $text.=$para."\n";
    $para='';
}

//may need to improve the function: the first 5 words of image txts is same as those in para.
function imageTxt($paraArr, $i, $para, &$index){
    $count=0;
    $paraArr[$i]=$para;
    while($count<7){ //jumping 5 para
        $paraTmp=$paraArr[$count+$i];
        if(delimiter($index, $paraTmp, 0)){
            return false; // not image txt
        }
        $count++;
    }
    removeImage($index, $para); //image txt
    return true;
}
...
function seperateStr2(&$line, &$para, &$text){ //find the head and tail of line to match para
    if($para!=''){
        ...
        if($head && $tail){ //find match
            ...
            $line=$Tmp[1];
            return true;
        } else if($head && !$tail){ //line seperated, the head and tail of the line in two different paras
...
//            exit("Para tail does not exist in pdf file. The reason may be delimiter function cannot return correct para tail.");
    ...
    //image txt / out of order para / extra para
    return false;
}
...
function removeImage(&$index, $para){
    ...
    if($head==false){ //|| preg_match('/^'.$head.'/u', $index, second condition avoid infinite loop
        $index='';
    }else{ //the head separates the image txt and contents
...
function compare($para, $index, $flag){
    ...
    while($validLen<5 && mb_strlen($para)>$next){
        ...
        if(count(preg_split('/'.$border.'/u', $index))==1){ //cannot find match
            ...
            if(count(preg_split('/'.$tmp.'/u', $index))>=2){ //if supplement is correct
                $border=$tmp;
            }else{
...
function checkSpace($border, $newChar, $index, $flag){
    ...
    if($flag===0){ // if para and index has white space in same position, append/prepend newChar to border
        return $border.preg_quote($newChar);
    }else{
        return preg_quote($newChar).$border;
    }
}
