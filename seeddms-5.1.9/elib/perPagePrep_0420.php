...
// remove page numbers, empty paragraph and decode the text to UTF-8 characters
    foreach ($paras as $para) { //paragraph
        ...
        if($catalogArr=catalogRegex($txt)){
            ...
            $txt = preg_replace('/[\x{2004}\x{3000}\t]/u', " ", $txt); //syncronize space
...
function findLines($lines, $paras){
    ...
    while($i<count($paras) && $j<count($lines)){
        $hitLine=seperateStr($line, $para, $text);
        if(!$hitLine || $line===''){
            $line=$lines[++$j];
        }
        if($para===''){
...
function printPara($indexArr, $paraArr, $verID){
    ...
    while($i<count($paraArr) && $j<count($indexArr)){
        if(($hitbuf=checkBuf($bufArr, $para, $text)) !== true){
            if($j===74)
                $a=1;
            if(($hitPara=seperateStr($para, $index, $text, $bufArr)) !== true){
                $index=''; // image txt
            }
        }
        if($para===''){
...
function checkBuf(&$bufArr, &$para, &$text){
    foreach ($bufArr as $k => $buf) {
        if(($hitBuf=seperateStr($para, $buf, $text, $bufArr))===true){
            if($buf!==''){
               $bufArr[$k]=$buf; 
            }else{
...
function seperateStr(&$para, &$index, &$text, &$bufArr=null){
    if($index!==''){
        ...
        if($head && $tail!==false){ //find match
            $tmp=preg_split('/'.$tail.'/u', $index, 2);
            $text.=$para."\n";
            $index=$tmp[1];
            $para='';
            return true;
        }else if($head && $tail===false){ // para seperated
...
function outOrder($bufArr, $indexArr, &$j, &$para, &$text, &$index){
    $count=0;
    $newBufArr=$bufArr;
    while($count<5){ //jumping 5 para
        $indexTmp=$indexArr[$count+$j];
        if(seperateStr($para, $indexTmp, $text)){
            $index=$indexTmp;
            $newBufArr[]=$paraTmp;
            return $newBufArr;
        }else{
