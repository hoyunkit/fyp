...
class SeedDMS_Session {
	...
	function load($id) { /* {{{ */
		...
		if (count($resArr) == 0)
			return false;
		$this->id = $id;
		$this->data = array('userid'=>$resArr[0]['userID'], 'theme'=>$resArr[0]['theme'], 'lang'=>$resArr[0]['language'], 'id'=>$resArr[0]['id'], 'lastaccess'=>$resArr[0]['lastAccess'], 'su'=>$resArr[0]['su'], 'nightmode'=>$resArr[0]['nightmode']);
	...
	function create($data) { /* {{{ */
		...
		$id = md5($id);
		$lastaccess = time();
		$queryStr = "INSERT INTO `tblSessions` (`id`, `userID`, `lastAccess`, `theme`, `language`, `su`, `nightmode`) ".
		  "VALUES ('".$id."', ".$data['userid'].", ".$lastaccess.", '".$data['theme']."', '".$data['lang']."', 0, 0)";
	 ...
	 * @return boolean true if successful otherwise false
	 */
        function setNightmode($tmp){
            $queryStr = "UPDATE `tblSessions` SET `nightmode` = " . $tmp . " WHERE `id` = " . $this->db->qstr($this->id);
		if (!$this->db->getResult($queryStr))
			return false;
		return true;
        }
