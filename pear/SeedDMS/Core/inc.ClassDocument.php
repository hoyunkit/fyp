...
class SeedDMS_Core_Document extends SeedDMS_Core_Object { /* {{{ */
	...
	function __construct($id, $name, $comment, $date, $expires, $ownerID, $folderID, $inheritAccess, $defaultAccess, $locked, $keywords, $sequence, $type) { /* {{{ */
	...
	public static function getInstance($id, $dms) { /* {{{ */
		...
		$document = new $classname($resArr["id"], $resArr["name"], $resArr["comment"], $resArr["date"], $resArr["expires"], $resArr["owner"], $resArr["folder"], $resArr["inheritAccess"], $resArr["defaultAccess"], $lock, $resArr["keywords"], $resArr["sequence"], $resArr["type"]);
		$document->setDMS($dms);
		return $document;
	} /* }}} */
	...
	function hasExpired() { /* {{{ */
        ...
        function setType($type){
            $db = $this->_dms->getDB();
            $queryStr = "UPDATE `tblDocuments` SET `type` = " . (int) $type . " WHERE `id` = " . $this->_id;
            if (!$db->getResult($queryStr))
                    return false;
...
//	} /* }}} */
	...
	function addAccess($mode, $userOrGroupID, $isUser)
	{
		...
		// Handle user's document access
		if ($isUser) {
			$queryStr1 = "SELECT `followGroup` FROM `tblACLs` WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = " . (int) $userOrGroupID . " AND `groupID` = -1";
			$result1 = $db->getResultArray($queryStr1);
			if ($result1) {
				$followGroup = $result1[0]['followGroup'];
				// If user's document access follow group
				if ($followGroup == 1) {
					// Save the selected document access in base document access
					$queryStr2 = "SELECT * FROM `tblDocumentAccessBase` WHERE `userID` = " . $userOrGroupID . " AND `documentID` = " . $this->_id;
					$result2 = $db->getResultArray($queryStr2);
					if ($result2) {
						$queryStr3 = "UPDATE `tblDocumentAccessBase` SET `baseMode` = " . $mode . " WHERE `userID` = " . $userOrGroupID . " AND `documentID` = " . $this->_id;
						$result3 = $db->getResult($queryStr3);
					} else {
						$queryStr4 = "INSERT INTO `tblDocumentAccessBase` (`userID`, `documentID`, `baseMode`) VALUES (" . $userOrGroupID . ", " . $this->_id . ", " . $mode . ")";
						$result4 = $db->getResult($queryStr4);
					}
				} else {
					// If user's document access does not follow group
					$queryStr5 = "UPDATE `tblACLs` SET `mode` = " . $mode . " WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = " . $userOrGroupID . " AND `groupID` = -1";
					$result5 = $db->getResult($queryStr5);

					// Save the selected document access in base document access
					$queryStr6 = "SELECT * FROM `tblDocumentAccessBase` WHERE `userID` = " . $userOrGroupID . " AND `documentID` = " . $this->_id;
					$result6 = $db->getResultArray($queryStr6);
					if ($result6) {
						$queryStr7 = "UPDATE `tblDocumentAccessBase` SET `baseMode` = " . $mode . " WHERE `userID` = " . $userOrGroupID . " AND `documentID` = " . $this->_id;
						$result7 = $db->getResult($queryStr7);
					} else {
						$queryStr8 = "INSERT INTO `tblDocumentAccessBase` (`userID`, `documentID`, `baseMode`) VALUES (" . $userOrGroupID . ", " . $this->_id . ", " . $mode . ")";
						$result8 = $db->getResult($queryStr8);
					}
				}
			} else {
				// Get the selected document access directly
				$queryStr9 = "INSERT INTO `tblACLs` (`target`, `targetType`, `userID`, `groupID`, `mode`, `followGroup`) VALUES (" . $this->_id . ", " . T_DOCUMENT . ", " . $userOrGroupID . ", " . -1 . ", " . $mode . ", " . 0 . ")";
				$result9 = $db->getResult($queryStr9);

				// Save the selected document access in base document access
				$queryStr10 = "SELECT * FROM `tblDocumentAccessBase` WHERE `userID` = " . $userOrGroupID . " AND `documentID` = " . $this->_id;
				$result10 = $db->getResultArray($queryStr10);
				if ($result10) {
					$queryStr11 = "UPDATE `tblDocumentAccessBase` SET `baseMode` = " . $mode . " WHERE `userID` = " . $userOrGroupID . " AND `documentID` = " . $this->_id;
					$result11 = $db->getResult($queryStr11);
				} else {
					$queryStr12 = "INSERT INTO `tblDocumentAccessBase` (`userID`, `documentID`, `baseMode`) VALUES (" . $userOrGroupID . ", " . $this->_id . ", " . $mode . ")";
					$result12 = $db->getResult($queryStr12);
				}
			}
		} else {
			// Hanlde group's document access
			...
			// Determine whether the document access mode currently owned by the user is larger than the document access mode assigned to the group and update it
			foreach ($groupMemberCollection as $key => $value) {
				...
				if ($result14) {
					foreach ($result14 as $key => $val) {
						$groupid = $val['groupID'];
						$queryStr15 = "SELECT `mode` FROM `tblACLs` WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = -1 AND `groupID` = " . $groupid . " AND `followGroup` = 0";
					...
					// Update group member's document access
					$queryStr16 = "SELECT `mode` FROM `tblACLs` WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = " . $value . " AND `groupID` = -1";
					$result16 = $db->getResultArray($queryStr16);
					if ($result16) {
						$queryStr17 = "UPDATE `tblACLs` SET `followGroup` = 1, `mode` = " . $newMode . " WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = " . $value . " AND `groupID` = -1";
						$result17 = $db->getResult($queryStr17);
					} else {
						$queryStr18 = "INSERT INTO `tblACLs` (`target`, `targetType`, `userID`, `groupID`, `mode`, `followGroup`) VALUES (" . $this->_id . ", " . T_DOCUMENT . ", " . $value . ", " . -1 . ", " . $newMode . ", " . 1 . ")";
						$result18 = $db->getResult($queryStr18);
					}
				} else {
					// Update group member's document access
					$queryStr19 = "SELECT `mode` FROM `tblACLs` WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = " . $value . " AND `groupID` = -1";
					$result19 = $db->getResultArray($queryStr19);
					if ($result19) {
						$queryStr20 = "UPDATE `tblACLs` SET `followGroup` = 1, `mode` = " . $newMode . " WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = " . $value . " AND `groupID` = -1";
						$result20 = $db->getResult($queryStr20);
					} else {
						$queryStr21 = "INSERT INTO `tblACLs` (`target`, `targetType`, `userID`, `groupID`, `mode`, `followGroup`) VALUES (" . $this->_id . ", " . T_DOCUMENT . ", " . $value . ", " . -1 . ", " . $newMode . ", " . 1 . ")";
						$result21 = $db->getResult($queryStr21);
					}
				}
			}

			// Update group document access
			$queryStr22 = "SELECT `mode` FROM `tblACLs` WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = -1 AND `groupID` = " . $userOrGroupID . " AND `followGroup` = 0";
			$result22 = $db->getResultArray($queryStr22);
			if ($result22) {
				$queryStr23 = "UPDATE `tblACLs` SET `mode` = " . $mode . " WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = -1 AND `groupID` = " . $userOrGroupID . " AND `followGroup` = 0";
				$result23 = $db->getResult($queryStr23);
			} else {
				$queryStr24 = "INSERT INTO `tblACLs` (`target`, `targetType`, `userID`, `groupID`, `mode`, `followGroup`) VALUES (" . $this->_id . ", " . T_DOCUMENT . ", " . -1 . ", " . $userOrGroupID . ", " . $mode . ", " . 0 . ")";
...
//	} /* }}} */
	...
	{ /* {{{ */
		...
		// Handle user
		if ($isUser) {
			// Clear the base document access
			$queryStr1 = "DELETE FROM `tblDocumentAccessBase` WHERE `userID` = " . (int)$userOrGroupID . " AND `documentID` = " . $this->_id;
			$result1 = $db->getResult($queryStr1);

			// Delete user's document access in table tblACLs
			$queryStr2 = "DELETE FROM `tblACLs` WHERE `targetType` = " . T_DOCUMENT . " AND `target` = " . $this->_id . " AND " . $userOrGroup . " = " . (int) $userOrGroupID;
			$result2 = $db->getResult($queryStr2);
		} else {
			// Handle group
			// 1. 尋找此group的成員 
			// 2. 尋找這些成員除了此group以外followGroup爲1的group
			// 3. 如果不存在，則從base document access裏尋找是否有backup。 如果存在，則尋找這些group在此document的access mode
			// 4. 選取最高等級的document access mode給對應的user
			// 5. 在tblACLs刪除所選擇group的document access mode
			...
			if ($result3) {
				...
				foreach ($result3 as $key => $val) {
					...
					if ($result4) {
						// Get the higher order document access mode
						foreach ($result4 as $key => $val) {
							$getGroupId = $val['groupID'];
							$queryStr5 = "SELECT `mode` FROM `tblACLs` WHERE `groupID` = " . $getGroupId . " AND `userID` = -1 AND `followGroup` = 0 AND `targetType` = " . T_DOCUMENT . " AND `target` = " . $this->_id;
						...
						// 0 means that this user already follow group, but group does not defind any document access
						if ($newMode == 0) {
							$queryStr6 = "SELECT `baseMode` FROM `tblDocumentAccessBase` WHERE `userID` = " . $getUserId . " AND `documentID` = " . $this->_id;
							$result6 = $db->getResultArray($queryStr6);
							if ($result6) {
								// Restore base document access
								foreach ($result6 as $key => $val) {
									$queryStr7 = "UPDATE `tblACLs` SET `followGroup` = 0, `mode` = " . (int)$val['baseMode'] . " WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = " . $getUserId . " AND `groupID` = -1";
									$result7 = $db->getResult($queryStr7);
								}
							} else {
								// No base document access to restored, so delete document access record in tblACLs table
								$queryStr8 = "DELETE FROM `tblACLs` WHERE `targetType` = " . T_DOCUMENT . " AND `target` = " . $this->_id . " AND `userID` = " . $getUserId . " AND `groupID` = -1";
								$result8 = $db->getResult($queryStr8);
							}
						} else {
							// Update user's document access with new mode
							$queryStr9 = "UPDATE `tblACLs` SET `mode` = " . $newMode . " WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = " . $getUserId . " AND `groupID` = -1 AND `followGroup` = 1";
							$result9 = $db->getResult($queryStr9);
						}
					} else {
						// No other groups, so find the base document access
						$queryStr10 = "SELECT `baseMode` FROM `tblDocumentAccessBase` WHERE `userID` = " . $getUserId . " AND `documentID` = " . $this->_id;
						$result10 = $db->getResultArray($queryStr10);
						if ($result10) {
							foreach ($result10 as $key => $val) {
								$queryStr11 = "UPDATE `tblACLs` SET `followGroup` = 0, `mode` = " . (int)$val['baseMode'] . " WHERE `target` = " . $this->_id . " AND `targetType` = " . T_DOCUMENT . " AND `userID` = " . $getUserId . " AND `groupID` = -1";
								$result11 = $db->getResult($queryStr11);
							}
						} else {
							$queryStr12 = "DELETE FROM `tblACLs` WHERE `targetType` = " . T_DOCUMENT . " AND `target` = " . $this->_id . " AND `userID` = " . $getUserId . " AND `groupID` = -1";
							$result12 = $db->getResult($queryStr12);
						}
					}
				}
			...
			// Delete group's document access in table tblACLs
			$queryStr13 = "DELETE FROM `tblACLs` WHERE `targetType` = " . T_DOCUMENT . " AND `target` = " . $this->_id . " AND " . $userOrGroup . " = " . (int) $userOrGroupID;
...
//							//file_put_contents("/home/www-data/seeddms51x/debug.txt", $caller[0].$caller[1].' ', FILE_APPEND);
//							if($caller[0]=='out'){
...
//								$mode = $groupAccess->getMode();
//							}
//							//file_put_contents("/home/www-data/seeddms51x/debug.txt", $groupAccess->getGroup()->getName().':'.$mode.'\n', FILE_APPEND);
//						}else{
//							$mode = $groupAccess->getMode();
//						}
//					}
...
//	} /* }}} */
	...
	{ /* {{{ */
        ...
        function checkPrivAccess($user, $perm) { /* {{{ */
            /* Administrators have unrestricted access */
            if ($user->isAdmin()) return 1;
            /* The owner of the document has unrestricted access */
            if ($user->getID() == $this->_ownerID) return 1;
            /* Check ACLs */
            $accessList = $this->getAccessList();
            if (!$accessList) return false;
            /** @var SeedDMS_Core_UserAccess $userAccess */
            ...
            if ($user->isGuest() || $mode < $perm || !$priv) {
               return 0;
            }
            return 1;
	}
	 ...
	 */
	function addContent($comment, $user, $tmpFile, $orgFileName, $fileType, $mimeType, $flipDir, $orientation, $reviewers=array(), $approvers=array(), $version=0, $attributes=array(), $workflow=null) { /* {{{ */
		$db = $this->_dms->getDB();
		...
		$db->startTransaction();
		$queryStr = "INSERT INTO `tblDocumentContent` (`document`, `version`, `comment`, `date`, `createdBy`, `dir`, `orgFileName`, `fileType`, `mimeType`, `fileSize`, `checksum`, `flipDir`, `orientation`) VALUES ".
						"(".$this->_id.", ".(int)$version.",".$db->qstr($comment).", ".$db->getCurrentTimestamp().", ".$user->getID().", ".$db->qstr($dir).", ".$db->qstr($orgFileName).", ".$db->qstr($fileType).", ".$db->qstr($mimeType).", ".$filesize.", ".$db->qstr($checksum).", ".$db->qstr($flipDir).", ".$orientation.")";
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
	...
	function getLatestContent() { /* {{{ */
		if (!$this->_latestContent) {
			...
			foreach ($resArr as $row) {
				if (!$this->_latestContent) {
				    /** @var SeedDMS_Core_DocumentContent $content */
					$content = new $classname($row["id"], $this, $row["version"], $row["comment"], $row["date"], $row["createdBy"], $row["dir"], $row["orgFileName"], $row["fileType"], $row["mimeType"], $row['fileSize'], $row['checksum'], $row['flipDir'], $row['orientation']);
					if($user) {
	...
	function getDocumentFile($ID) { /* {{{ */
        ...
        function getDocumentWordFiles($version=0) { /* {{{ */
            $db = $this->_dms->getDB();
            $queryStr = "SELECT * FROM `tblDocumentWordFiles` WHERE `document` = " . $this->_id;
            if($version) {
            ...
            $wordFiles = array();
            foreach ($resArr as $row) {
                $wordFile = new SeedDMS_Core_DocumentWordFile($row["id"], $this, $row["userID"], $row["comment"], $row["date"], $row["dir"], $row["fileType"], $row["mimeType"], $row["orgFileName"], $row["name"], $row["version"], $row["public"]);
    //            if($wordFile->getAccessMode($user) >= M_READ){
                        array_push($wordFiles, $wordFile);
    //            }
            }
        return $wordFiles;
        } /* }}} */
        
        function getDocumentWordFile($ID) { /* {{{ */
            $db = $this->_dms->getDB();
            if (!is_numeric($ID)) return false;
            $queryStr = "SELECT * FROM `tblDocumentWordFiles` WHERE `document` = " . $this->_id ." AND `id` = " . (int) $ID;
            $resArr = $db->getResultArray($queryStr);
            if ((is_bool($resArr) && !$resArr) || count($resArr)==0) return false;
            $resArr = $resArr[0];
            $wordFile = new SeedDMS_Core_DocumentWordFile($resArr["id"], $this, $resArr["userID"], $resArr["comment"], $resArr["date"], $resArr["dir"], $resArr["fileType"], $resArr["mimeType"], $resArr["orgFileName"], $resArr["name"],$resArr["version"],$resArr["public"]);
            $user = $this->_dms->getLoggedInUser();
            if($wordFile->getAccessMode($user) >= M_READ)
                    return $wordFile;
            return null;
	...
	function addDocumentFile($name, $comment, $user, $tmpFile, $orgFileName,$fileType, $mimeType,$version=0,$public=1) { /* {{{ */
        ...
        function addDocumentWordFile($name, $comment, $user, $tmpFile, $orgFileName,$fileType, $mimeType,$version=0,$public=1) { /* {{{ */
            $db = $this->_dms->getDB();
            $dir = $this->getDir();
            $queryStr = "INSERT INTO `tblDocumentWordFiles` (`comment`, `date`, `dir`, `document`, `fileType`, `mimeType`, `orgFileName`, `userID`, `name`, `version`, `public`) VALUES ".
                    "(".$db->qstr($comment).", ".$db->getCurrentTimestamp().", ".$db->qstr($dir).", ".$this->_id.", ".$db->qstr($fileType).", ".$db->qstr($mimeType).", ".$db->qstr($orgFileName).",".$user->getID().",".$db->qstr($name).", ".((int) $version).", ".($public ? 1 : 0).")";
            if (!$db->getResult($queryStr)) return false;
            $id = $db->getInsertID('tblDocumentWordFiles');
            $wordFile = $this->getDocumentWordFile($id);
            if (is_bool($wordFile) && !$wordFile) return false;
            // copy file
            if (!SeedDMS_Core_File::makeDir($this->_dms->contentDir . $dir)) return false; // check if folder exist
            if($this->_dms->forceRename)
                $err = SeedDMS_Core_File::renameFile($tmpFile, $this->_dms->contentDir . $wordFile->getPath());
            else
                $err = SeedDMS_Core_File::copyFile($tmpFile, $this->_dms->contentDir . $wordFile->getPath());
            if (!$err) return false;
            return $wordFile;
        }
        function removeDocumentWordFile($ID) { /* {{{ */
		$db = $this->_dms->getDB();

		if (!is_numeric($ID)) return false;

		$file = $this->getDocumentWordFile($ID);
		if (is_bool($file) && !$file) return false;
		...
		$queryStr = "DELETE FROM `tblDocumentWordFiles` WHERE `document` = " . $this->getID() . " AND `id` = " . (int) $ID;
		if (!$db->getResult($queryStr))
			return false;

		unset ($this->_documentWordFiles);
	...
	function remove() { /* {{{ */
		...
		if (!$db->getResult($queryStr)) {
			...
			return false;
                ...
                $queryStr = "DELETE FROM `tblDocumentWordFiles` WHERE `document` = " . $this->_id;
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
...
class SeedDMS_Core_DocumentContent extends SeedDMS_Core_Object { /* {{{ */
	...
	function __construct($id, $document, $version, $comment, $date, $userID, $dir, $orgFileName, $fileType, $mimeType, $fileSize=0, $checksum='', $flipDir='ltr', $orientation=1) { /* {{{ */
		parent::__construct($id);
	...
	function getDocument() { return $this->_document; }
        function getFlipDir() { return $this->_flipDir;}
...
class SeedDMS_Core_DocumentWordFile { /* {{{ */
	/**
	 * @var integer internal id of document file
	 */
	protected $_id;
	...
	/**
	 * SeedDMS_Core_DocumentWordFile constructor.
	 * @param $id
	 * @param $document
	 * @param $userID
	 * @param $comment
	...
	function setComment($newComment) { /* {{{ */
		...
		$queryStr = "UPDATE `tblDocumentWordFiles` SET `comment` = ".$db->qstr($newComment)." WHERE `document` = ".$this->_document->getId()." AND `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	function setDate($date) { /* {{{ */
		...
		$queryStr = "UPDATE `tblDocumentWordFiles` SET `date` = " . (int) $date . " WHERE `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
		$this->_date = $date;
		return true;
	...
	function setName($newName) { /* {{{ */
		...
		$queryStr = "UPDATE `tblDocumentWordFiles` SET `name` = ".$db->qstr($newName)." WHERE `document` = ".$this->_document->getId()." AND `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	 ...
	 */
	function getPath() {
		return $this->_document->getDir() . $this->getVersion() . $this->_fileType;
	}
	...
	function setVersion($newVersion) { /* {{{ */
		...
		$queryStr = "UPDATE `tblDocumentWordFiles` SET `version` = ".(int) $newVersion." WHERE `document` = ".$this->_document->getId()." AND `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
	...
	function setPublic($newPublic) { /* {{{ */
		...
		$queryStr = "UPDATE `tblDocumentWordFiles` SET `public` = ".($newPublic ? 1 : 0)." WHERE `document` = ".$this->_document->getId()." AND `id` = ". $this->_id;
		if (!$db->getResult($queryStr))
			return false;
