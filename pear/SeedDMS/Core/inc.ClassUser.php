...
class SeedDMS_Core_User { /* {{{ */
	 ...
	 */
	function __construct($id, $login, $pwd, $fullName, $email, $language, $theme, $comment, $role, $isHidden=0, $isDisabled=0, $pwdExpiration='', $loginFailures=0, $quota=0, $homeFolder=null, $firstLogin=1) {
		$this->_id = $id;
		$this->_login = $login;
		$this->_pwd = $pwd;
	...
	public static function getInstance($id, $dms, $by='', $email='') { /* {{{ */
		...
		$user = new self($resArr["id"], $resArr["login"], $resArr["pwd"], $resArr["fullName"], $resArr["email"], $resArr["language"], $resArr["theme"], $resArr["comment"], $resArr["role"], $resArr["hidden"], $resArr["disabled"], $resArr["pwdExpiration"], $resArr["loginfailures"], $resArr["quota"], $resArr["homefolder"], $resArr["firstLogin"]);
		$user->setDMS($dms);
		return $user;
	} /* }}} */
	...
	public static function getAllInstances($orderby, $dms) { /* {{{ */
		...
		for ($i = 0; $i < count($resArr); $i++) {
			...
			$user = new self($resArr[$i]["id"], $resArr[$i]["login"], $resArr[$i]["pwd"], $resArr[$i]["fullName"], $resArr[$i]["email"], (isset($resArr[$i]["language"])?$resArr[$i]["language"]:NULL), (isset($resArr[$i]["theme"])?$resArr[$i]["theme"]:NULL), $resArr[$i]["comment"], $resArr[$i]["role"], $resArr[$i]["hidden"], $resArr[$i]["disabled"], $resArr[$i]["pwdExpiration"], $resArr[$i]["c"], $resArr[$i]["quota"], $resArr[$i]["homefolder"]);
			$user->setDMS($dms);
			$users[$i] = $user;
		}
	...
	function addLoginFailure() { /* {{{ */
        ...
        function setFirstLogin(){
            $db = $this->_dms->getDB();
            $this->_firstLogin=0;
            $queryStr = "UPDATE `tblUsers` SET `firstLogin` = " . $this->_firstLogin . " WHERE `id` = " . $this->_id;
            if (!$db->getResult($queryStr))
                    return false;
	...
	function getReadingHistory(){
		$db = $this->_dms->getDB();
		$queryStr = "SELECT documentid FROM tblUserReadingHistory WHERE userid = ".$this->_id;
	...
	function getReadingDetail($docid){
		$db = $this->_dms->getDB();
		$queryStr = "SELECT * FROM tblUserReadingHistory WHERE userid = ".$this->_id ."&& documentid=".$docid;
		$resArr = $db->getResultArray($queryStr);
		return $resArr[0];
	}
	
	function addReadingHistory($docid){
		$db = $this->_dms->getDB();
		$queryStr = "INSERT INTO `tblUserReadingHistory`(`userid`, `documentid`, `page`, `tabStatus`, `disPage`) VALUES (".$this->_id.",".$docid.", 1, 0, 1)";
		$resArr = $db->getResultArray($queryStr);
	}
        /*
        function addReadingHistory($docid){
            ...
            if(!$resArr){
                ...
                if(in_array($docid, $arr)){
                    $queryStr = "DELETE FROM `tblUserReadingHistory` WHERE `userid` = ".$this->_id."&& `documentid`=".$docid;
                    $resArr = $db->getResultArray($queryStr);
                }elseif(sizeof($arr)==5){
                    $queryStr = "DELETE FROM `tblUserReadingHistory` WHERE `userid` = ".$this->_id."&& `documentid`=$arr[0]";
                    $resArr = $db->getResultArray($queryStr);
                }
            }
            $queryStr = "INSERT INTO `tblUserReadingHistory`(`userid`, `documentid`) VALUES (".$this->_id.",".$docid.")";
            $resArr = $db->getResultArray($queryStr);
        }*/
        function updateReadingHistory($index, $docid, $entry,$mode){
            $db = $this->_dms->getDB();
            switch($index){
                case 0:
                    $queryStr = "UPDATE tblUserReadingHistory SET page='".$entry."', mode='".$mode."' WHERE userid='".$this->_id."' AND documentid='".$docid."'";
                    break;
                case 1:
                    $queryStr = "UPDATE tblUserReadingHistory SET tabStatus=".$entry.",mode=".$mode." WHERE userid = ".$this->_id."&& documentid=".$docid;
                    break;
                case 2:
                    $queryStr = "UPDATE tblUserReadingHistory SET disPage=".$entry.",mode=".$mode." WHERE userid = ".$this->_id."&& documentid=".$docid;
                    break;
            }
            $resArr = $db->getResultArray($queryStr);
            return $resArr;
        ...
        function getUserNoti() {
            $db = $this->_dms->getDB();
            $queryStr = "SELECT * FROM `tblUserNoti` WHERE `userid` = ". $this->_id." && `status`=1 ORDER by `id` DESC";
            //file_put_contents("/home/www-data/seeddms51x/debug.txt", $queryStr, FILE_APPEND);
            $res = $db->getResultArray($queryStr);
            $queryStr = "SELECT * FROM `tblUserNoti` WHERE `userid` = ". $this->_id." && `status`=2 ORDER by `id` DESC";
            $res2 = $db->getResultArray($queryStr);
            $res=array_merge($res, $res2);
            $str='<table class="table table-hover" id="notitbl" style="margin-bottom:0;">'; // remove table margin at bottom
            foreach ($res as $row) {
                $str=$str.'<tr data-id="'.$row['id'].'" style="cursor:pointer;';
                if($row['status']==2){
                    $str=$str.'color: darkgrey;';
                }
                $str=$str.'" data-href="'.$row['link'].'"><th style="padding:0;">'; //remove cell padding
                $msg=explode("<notibreak>", $row["msg"]);
                if($row['type']==1){
                    $str=$str.$this->_dms->GetUserNameByID($msg[0])[0]['login'] .' reply your notes <br>';
                    $str=$str.'<span style="font-weight:normal;">&nbsp&nbsp'.$msg[1].'...</span>';
                }
                else if($row['type']==2){
                    $str=$str.$this->_dms->GetUserNameByID($msg[0])[0]['login'] .' send advice to you <br>';
                    $str=$str.'<span style="font-weight:normal;">&nbsp&nbsp'.$msg[1].'...</span>';
                }
                $date=explode(" ", $row["time"]);
                $str=$str.'</th><th style="font-weight:normal;padding:0;">'; //remove cell padding
                if(!strcmp(date("Y-m-d"),$date[0])){
                    //var_dump(date("H:i:s"));
                    //var_dump($date[1]);
                    $min=(strtotime(date("H:i:s"))-strtotime($date[1]))/60;
                    $min=round($min);
                    //var_dump($min);
                    if($min<1){
                        $str2=round($min*60).'s';
                    }else if($min>59){
                        $str2=round($min/60).'hour';
                    }else{
                        $str2=$min.'min';
                    }
                    $str=$str.$str2.' before';
